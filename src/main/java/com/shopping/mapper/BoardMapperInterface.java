...
public interface BoardMapperInterface {
    ...
    @Update("update boards set writer=#{board.writer}, subject=#{board.subject}, description=#{board.description} where no=#{board.no} ") // 게시물 수정하기
    int Update(@Param("board") final Board bean) ;

    @Delete("delete from boards where no = #{no}") // 게시물 삭제하기
